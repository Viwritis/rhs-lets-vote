...
import pl.redheadsolutions.lg.lets.vote.config.KafkaProperties;

class LetsVoteKafkaResourceIT {
    ...
    void setup() {
        ...
        LetsVoteKafkaResource kafkaResource = new LetsVoteKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/lets-vote-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/lets-vote-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
